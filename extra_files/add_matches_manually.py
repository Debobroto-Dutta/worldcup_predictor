#!/usr/bin/env python3
"""
Simple script to manually add World Cup 2026 matches to the database
Run this script: python add_matches_manually.py
"""
import sys
import os
...
def add_matches():
    """Add all FIFA World Cup 2026 matches to the database"""
    
    with app.app_context():
        # Check if matches already exist
        existing_count = Match.query.count()
        if existing_count > 0:
            print(f"\n⚠️  Warning: Database already has {existing_count} matches.")
            response = input("Do you want to delete all existing matches and add fresh data? (yes/no): ")
        ...
        print(f"\n✅ Successfully added {len(matches)} matches to the database!")
        print("\nMatch breakdown:")
        print("  - 72 Group Stage matches")
        print("  - 16 Round of 32 matches")
        print("  - 8 Round of 16 matches")
        print("  - 4 Quarter Final matches")
        print("  - 2 Semi Final matches")
        print("  - 1 Third Place match")
        print("  - 1 Final match")
        print("\n🎉 Your World Cup Predictor is ready to use!")
